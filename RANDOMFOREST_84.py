...
# Step 6: Define and train the RandomForest model
rf_model = RandomForestClassifier(n_estimators=40, max_depth=20, min_samples_split=8, min_samples_leaf=4, random_state=42)
rf_model.fit(X_train, y_train)

# Step 7: Apply Recursive Feature Elimination with the best model
rfe = RFE(estimator=rf_model, n_features_to_select=10)  # Select top 10 features
rfe.fit(X_train, y_train)

# Step 8: Perform cross-validation
kf = KFold(n_splits=5, shuffle=True, random_state=42)
cross_val_accuracies = cross_val_score(rf_model, X_train, y_train, cv=kf, scoring='accuracy')
...
# Step 13: Test function to make predictions with new audio file
